...
def is_prioritized(replay_buffer: PrioritizedReplayBuffer):
    important_index = 2
    for i in range(10):
        replay_buffer.add(ReplayBufferEntry(np.array([i]), np.array([i]), i, i, False))

    replay_buffer.update_priorities(list(range(10)), [0.5 if i == important_index else 0.1 for i in range(10)])
    ...
    last_index = 11
    replay_buffer.update_priorities(list(range(10)), [0.1 for _ in range(10)])
    replay_buffer.add(ReplayBufferEntry(np.array([last_index]), np.array([last_index]), last_index, last_index, False))
    dist = {}
    for _ in range(100):
...
def test_important_transitions_are_prioritized():
    ...
    replay_buffer = PrioritizedReplayBuffer(prb)
