...
class DqnMemoryAgent(ABC):
    def __init__(self,
        ...
        self.memory = Memory(hp.memory_len)
    ...
    def memorize(self, s: np.ndarray, m: torch.Tensor, s_: np.ndarray, a: int, r: float, final: bool) -> None:
        self.memory.add(MemoryEntry(s, m, s_, a, r, final))
    ...
    def learn(self, batch_size: int, random: bool = True) -> None:
        ...
        batch_m = torch.cat([m.m for m in memories], 0).requires_grad_(True)
