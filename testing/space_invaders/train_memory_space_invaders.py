...
class SpaceInvadersMemoryActionEstimator(torch.nn.Module):
    def __init__(self, width: int, height: int, out_size: int):
        super(SpaceInvadersMemoryActionEstimator, self).__init__()
        ...
        self.head = torch.nn.Linear(w*h*2, out_size)
    ...
    def to(self, device: str):
        self.device = device
        return super(SpaceInvadersMemoryActionEstimator, self).to(device)
...
def main():
    ...
    class SpaceInvadersDqnAgent(DqnMemoryAgent):
        @staticmethod
        def action_estimator_factory() -> torch.nn.Module:
            return SpaceInvadersMemoryActionEstimator(
        ...
        def memory_init(self) -> torch.Tensor:
            action_estimator: SpaceInvadersMemoryActionEstimator = self.action_estimator
    ...
    trainer.train(lambda progress: progress.tries >= 100000)
